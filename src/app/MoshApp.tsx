...
import { Download, RotateCcw, Dices, AlertTriangle, X, Github, Info, Heart } from 'lucide-react'
...
export default function MoshApp() {
  ...
  // Desktop: write the result to a path the user chooses (native Save dialog).
  const saveResult = useCallback(async () => {
    if (!resultUrl) return
    const { save } = await import('@tauri-apps/plugin-dialog')
    const path = await save({
      defaultPath: `dmosh-${effect}.mp4`,
      filters: [{ name: 'Video', extensions: ['mp4'] }],
    })
    if (!path) return
    const bytes = new Uint8Array(await (await fetch(resultUrl)).arrayBuffer())
  ...
  return (
    <div style={accentVars} className="flex h-screen flex-col bg-neutral-50 text-neutral-800">
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-neutral-200 bg-white px-4 py-2.5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={isDesktop ? 'About dmosh' : 'Native desktop build — faster, larger files. Buy it prebuilt or compile the source yourself.'}
          ...
          <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden p-4">
            {view === 'output' && resultUrl ? (
              ...
              <div className="text-center text-xs text-neutral-500">
                <MoshSprite />
                <div className="mt-4 text-neutral-300">no clip loaded</div>
            ...
            <div className="flex items-center gap-2">
              ...
              {status === 'done' && resultUrl && (
                <>
                  ...
                  {isDesktop ? (
                    <button type="button" onClick={saveResult} style={{ background: 'var(--accent)' }} className="flex items-center gap-1.5 rounded px-3 py-1.5 text-[11px] font-medium text-white hover:opacity-90">
      ...
      {desktopOpen &&
        (isDesktop ? (
          <AboutModal version={releaseVersion} onClose={() => setDesktopOpen(false)} />
        ) : (
          <DesktopModal onClose={() => setDesktopOpen(false)} />
        ))}
    </div>
  )
}
...
// A small datamosh mascot — a little monitor that blinks and occasionally
// glitches (RGB-split). Pure CSS/SVG, sits in the empty viewport for a bit of vibe.
function MoshSprite() {
  return (
    <div className="dmosh-sprite" aria-hidden>
      <svg width="84" height="84" viewBox="0 0 32 32" fill="none">
        <rect x="4" y="6" width="24" height="17" rx="3" stroke="var(--accent)" strokeWidth="1.5" fill="rgba(255,255,255,0.04)" />
        <rect className="eye" x="11" y="12" width="3" height="4" rx="1" fill="#e5e5e5" />
        <rect className="eye" x="18" y="12" width="3" height="4" rx="1" fill="#e5e5e5" />
        <rect x="13" y="19" width="6" height="1.6" rx="0.8" fill="#a1a1aa" />
        <rect x="14.5" y="23" width="3" height="3" fill="var(--accent)" />
        <rect x="10" y="26" width="12" height="1.6" rx="0.8" fill="#52525b" />
      </svg>
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="w-full max-w-sm rounded-lg border border-neutral-200 bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-mono text-sm font-semibold text-neutral-900">
              dmosh <span className="text-neutral-400">{version}</span>
            </h2>
            <p className="mt-1 text-[12px] leading-relaxed text-neutral-500">
              A datamosher you own. Open source (MIT) — this native build runs your machine's ffmpeg.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-2">
          <a href={SPONSOR_URL} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-md border border-neutral-200 px-3 py-2.5 text-[13px] text-neutral-800 transition-colors hover:border-neutral-300">
            <Heart size={14} style={{ color: 'var(--accent)' }} /> Sponsor dmosh on GitHub
          </a>
          <a href={GITHUB_URL} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-md border border-neutral-200 px-3 py-2.5 text-[13px] text-neutral-800 transition-colors hover:border-neutral-300">
            <Github size={14} /> Source on GitHub
          </a>
        </div>
        <p className="mt-3 text-center text-[10px] text-neutral-400">built by hand · funded by you</p>
      </div>
    </div>
  )
}
